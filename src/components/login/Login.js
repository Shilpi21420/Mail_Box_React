  ...
  const submithandler = async (event) => {
    ...
    try {
      const respose = await axios.post(
        "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCuHo9UgXa2xExWjFfpO6KVBL2zfXczNhw",
        Userdetails
      );
    ...
    if (emailref.current.value === "") {
      ...
      try {
        const response = await axios.post(
          "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyCuHo9UgXa2xExWjFfpO6KVBL2zfXczNhw",
          details
        );
