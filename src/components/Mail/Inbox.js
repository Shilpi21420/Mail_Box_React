...
export default function Inbox() {
  ...
  const viewMailHandler = async (myid) => {
    try {
      const res = await axios.patch(
        `https://mail-box-84ae4-default-rtdb.firebaseio.com/${SenderEmail}/inbox/${myid}.json`,
        { read: true }
      );
      if (res.status === 200) {
        settrigger(!trigger);
  ...
  const deleteMailHabdler = async (id) => {
    await axios.delete(
      `https://mail-box-84ae4-default-rtdb.firebaseio.com/${SenderEmail}/inbox/${id}.json`
    );
    settrigger(!trigger);
  };
  ...
  const httpdata = usehttp(
    `https://mail-box-84ae4-default-rtdb.firebaseio.com/${SenderEmail}/inbox.json`,
